...
/**
 * This class is a SINGLETON that will handle all the data to the PMV formula
 */
public class PMV {
    ...
    // Constructor
    public PMV(float tempIn, float tempOut, float humidityIn, float humidityOut) {

        double clo = 1;// clothing
        double met = 1.1; // metabolic rate
        double wme = 0; // external work
        double ta = (double) ((tempIn + tempOut) / 2); // air temperature average
        double tr = 0; // mean radiant temperature, set to 0 to allow compilation
        double vel = 0.1; // wind velocity
        double rh = (double) ((humidityIn + humidityOut) / 2); // relative humidity average
        ...
        while (Math.abs(xn - xf) > eps) {
            ...
            if (n > 150) {
                pmv = 99999;
                ppd = 100;
                break; // set default and exit while loop
            }
            ...
            ts = (.303 * Math.exp(-.036 * m)) + 0.28; // thermal sensation trans. coeff

            /* This is the calculated Predicted Mean Vote */
            pmv = ts * (mw - hl1 - hl2 - hl3 - hl4 - hl5 - hl6); // predicted mean vote
            /* Predicted Percentage dissatisfied */
            ppd = 100 - (95 * Math.exp((-.03353 * (Math.pow(pmv, 4))) - (.2179 * (Math.pow(pmv, 2)))));
